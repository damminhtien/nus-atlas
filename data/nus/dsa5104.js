  ...
  window.NUS_CONTENT.DSA5104 = {
    modules: [
      ...
      { id: "dsa5104-sql", title: "Querying", lessons: [
        { id: "dsa5104-sql-foundations", title: "SQL foundations: select, join, group", week: 3, minutes: 55, summary: "Write queries that are explicit about rows, joins, grouping, and the meaning of NULL.", objectives: ["Filter rows before aggregation", "Join on keys rather than row position", "Explain WHERE versus HAVING"], sourceRefs: [source("DSA5104/chapter1.pdf", 23), source("DSA5104/Homework Solutions/Ch03 SQL", 1)], visualIds: [], sections: [{ title: "Query order", body: String.raw`Reason conceptually as $\mathrm{FROM/JOIN}\prec\mathrm{WHERE}\prec\mathrm{GROUP\ BY}\prec\mathrm{HAVING}\prec\mathrm{SELECT}\prec\mathrm{ORDER\ BY}$. Use the SQL practice lab to run small examples against a browser-local sample database.` }], questions: [
    ...
    sqlPractice: {
      ...
      exercises: [
        ...
        { id: "sql-2", level: "Join", prompt: "Return each student with the department name.", starter: "SELECT s.name, d.name AS department\nFROM Student s\nJOIN Department d ON d.id = s.department_id\nORDER BY s.id;", expected: ["An|Data Science", "Binh|Data Science", "Chi|Computer Science", "Duc|Statistics"], explanation: String.raw`The join uses $\mathrm{Student.department\_id}\to\mathrm{Department.id}$.` },
