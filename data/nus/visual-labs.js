(function () {
  ...
  window.NUS_VISUAL_LABS = {
    ...
    "dsa5105-week1-derivations": {
      ...
      sourceRefs: [lecture(23, "ERM and population risk"), lecture(35, "Huber influence"), lecture(48, "ridge spectrum"), lecture(54, "logistic convexity"), reference("DSA5105/Lec1_exercises-solutions.pdf", "official exercise solutions")],
      exercises: [
        ...
        { id: "logistic", label: "Logistic convexity", prompt: "How does binary softmax become a convex logistic objective?", takeaway: "The score difference gives the decision rule; the non-negative second derivative gives convexity in that score.", steps: [["Score", "s=z_1-z_2", "Two-class softmax depends only on the relative logit difference."], ["Decision", "\\hat y=\\operatorname{sign}(s)", "The sign is the final class decision, separate from the training surrogate."], ["Loss", "\\ell(s,y)=\\log(1+e^{-ys})", "Use y in {-1,+1} to write binary cross-entropy compactly."], ["Curvature", "\\frac{\\partial^2\\ell}{\\partial s^2}=\\frac{e^{ys}}{(1+e^{ys})^2}\\ge0", "Positive curvature everywhere means the score-level loss is convex."]] },
        { id: "synthesis", label: "Synthesis map", prompt: "Can you connect data, fitting, diagnosis, and stabilization?", takeaway: "The equations are one pipeline: sample → objective → fitted model → generalization diagnosis → rank-aware regularization.", steps: [["Data", "\\mathcal D=\\{(x_i,y_i)\\}_{i=1}^{N}", "Name the observed sample and the oracle it approximates."], ["Fit", "(\\mathcal H,\\ell)\\to\\widehat R\\to\\hat f", "Choose a hypothesis class and loss before optimizing."], ["Diagnose", "\\text{approximation}+\\text{optimization}+\\text{generalization}", "Separate model capacity, algorithmic failure, and sample-to-population failure."], ["Stabilize", "\\text{rank deficiency}\\to\\text{regularization}\\to\\text{spectral filtering}", "Ridge suppresses weak directions instead of pretending they are well identified."]] }
      ],
