...
test("DSA5208 has a complete concept contrast set", () => {
  const state = loadLegacyState();
  const result = validateContrastDrills(state);
  assert.equal(result.ok, true, result.errors.join("\n"));
  assert.ok(DSA5208_REQUIRED_PAIRS.every(pair => state.content.DSA5208.modules.flatMap(module => module.lessons || []).some(lesson => (lesson.contrastDrills || []).some(drill => drill.pair === pair))));
});
