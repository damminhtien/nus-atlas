...
const appSource = fs.readFileSync("src/app/app-shell.js", "utf8");
...
test("access gate blocks the shell until the session passcode is accepted", () => {
  ...
  assert.ok(indexHtml.indexOf('src="src/core/access-gate.js') < indexHtml.indexOf('src="src/app/bootstrap.js'));
  ...
  assert.match(appSource, /config\.accessReady/);
