...
const META = {
  ...
  2: ['chapter outline', 'maps the chapter: structure of relational databases, database schema, keys, schema diagrams, relational query languages, and the relational algebra', 'skipping the outline and losing the narrative structure', 'orient', 'What are the six building blocks Chapter 2 introduces?', 'Relation structure, database schema, keys, schema diagrams, relational query languages, and the relational algebra.', 'Outline = roadmap.'],
  3: ['the instructor relation as a table', 'grounds the abstract relation in a concrete table with attributes (columns) and tuples (rows)', 'confusing the visual table layout with the mathematical relation', 'define', 'In the instructor relation, what do columns and rows represent?', 'Columns are attributes; rows are tuples of the relation.', 'Columns = attributes, rows = tuples.'],
  4: ['relation schema and instance', 'distinguishes the schema R = (A1, A2, ..., An) from the current instance (the set of tuples), a distinction used throughout the course', 'conflating the schema (structure) with an instance (data at one time)', 'compare', 'What is the difference between a relation schema and a relation instance?', 'The schema is the set of attributes R = (A1, ..., An); the instance is the current set of tuples stored in that relation.', 'Schema = structure; instance = data.'],
  5: ['attribute domains', 'defines the domain of an attribute as the set of allowed values, which constrains what data is legal', 'ignoring domain constraints when reasoning about valid tuples', 'define', 'What is the domain of an attribute?', 'The set of allowed values for that attribute.', 'Domain = allowed values.'],
  ...
  25: ['join with a matching condition', 'applies the condition instructor.ID = teaches.ID to the product to form the join', 'forgetting the equality condition that makes the join meaningful', 'apply', 'What condition turns instructor x teaches into a useful join?', 'Selecting tuples where instructor.ID = teaches.ID.', 'Match on ID.'],
  26: ['join expressed in SQL', 'shows the SQL code corresponding to the join condition', 'confusing the algebra join with the SQL syntax without mapping them', 'apply', 'How is the algebra join written in SQL?', 'As a query over instructor and teaches with a where clause equating instructor.ID and teaches.ID.', 'Where clause equates keys.'],
