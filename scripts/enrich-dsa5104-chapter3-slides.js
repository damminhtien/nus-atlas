...
const META = {
  ...
  37: ['string operations', 'introduces SQL string-matching with patterns like % and _', 'confusing % (any substring) with _ (single character)', 'define', 'What do % and _ mean in SQL LIKE patterns?', '% matches any substring (including empty); _ matches exactly one character.', 'Percent = many, underscore = one.'],
  ...
  62: ['set comparison in where', 'introduces comparison operators combined with subqueries, such as > some and > all', 'forgetting that set comparisons require some/all quantification', 'define', 'How do SQL comparisons extend to sets?', 'Using some (exists an element) and all (all elements) quantification after a comparison.', 'some vs all.'],
  63: ['the some clause', 'finds instructors with salary greater than that of some (at least one) instructor', 'confusing some with all', 'define', 'What does F > some r mean?', 'There exists a tuple t in r such that F > t.', 'At least one.'],
  64: ['definition of some', 'states F <comp> some r iff there exists t in r with F <comp> t', 'forgetting that some is existential quantification', 'define', 'Write the formal meaning of F <comp> some r.', 'F <comp> some r if and only if there exists t in r such that F <comp> t.', 'Existential.'],
  ...
  66: ['the all clause', 'finds instructors with salary greater than the salary of every instructor', 'confusing all with some', 'define', 'What does F > all r mean?', 'For every tuple t in r, F > t.', 'All tuples.'],
  67: ['definition of all', 'states F <comp> all r iff for all t in r, F <comp> t', 'forgetting that all is universal quantification', 'define', 'Write the formal meaning of F <comp> all r.', 'F <comp> all r if and only if for every tuple t in r, F <comp> t.', 'Universal.'],
  ...
  91: ['updates', 'gives a 5% salary raise to all instructors', 'forgetting that update without where changes every row', 'apply', 'How do you give every instructor a 5% raise?', 'update instructor set salary = salary * 1.05.', 'Scale all rows.'],
  ...
  94: ['conditional updates', 'raises salaries by 3% for instructors earning over $90,000 and 5% otherwise', 'forgetting to express conditional logic in update', 'apply', 'How do you apply different raises based on salary?', 'With a case statement in the set clause or two updates.', 'Case in set.'],
  95: ['case statement for conditional updates', 'rewrites the conditional raise using case', 'confusing case in update with case in select', 'apply', 'How does case express the conditional raise?', 'set salary = case when salary > 90000 then salary * 1.03 else salary * 1.05 end.', 'Conditional set.'],
...
function buildExplanation(meta, title, text, page) {
  const t = (text || '').replace(/\s+/g, ' ').trim();
  return {
    whatYouSee: `The slide titled "${title}" presents: ${t.slice(0, 220)}${t.length > 220 ? '...' : ''}`,
    whyItMatters: meta[1],
...
function main() {
  ...
  for (const slide of data.slides) {
    ...
    if (meta) {
      slide.explanation = buildExplanation(meta, slide.title, text, page);
