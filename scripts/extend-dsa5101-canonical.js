...
const commonQuestion = (id, lessonId, type, difficulty, skill, cognitiveLevel, estimatedSeconds, prompt, explanation, sourceRefs, extra = {}) => ({
  ...
  misconception: extra.misconception || ({
    'distinguish-linkages': 'Confusing the nearest cross-cluster pair used by single linkage with the farthest pair used by complete linkage.',
    'calculate-distortion': 'Using a median or arbitrary midpoint instead of the mean that minimizes squared error.',
    'reason-about-local-optima': 'Equating a non-increasing K-means objective with a globally optimal clustering.',
    'classify-bfr-state': 'Putting an uncertain point in DS before it has enough evidence, instead of keeping it in CS or RS.',
    'compare-cluster-representations': 'Assuming one centroid preserves non-spherical shape as well as several shrunk CURE representatives.',
    'evaluate-clusters': 'Treating purity 1 as proof of useful clustering without checking cluster count or task utility.',
    'normalize-ratings': 'Computing user similarity on raw ratings instead of deviations from each user mean.',
    'compute-dot-product': 'Replacing the latent dot product with a norm or dropping the negative coordinate contribution.',
    'separate-models': 'Blending a neighborhood-weighted prediction with a latent-factor dot product.',
    'diagnose-cold-start': 'Treating missing ratings as zeros instead of recognizing that a cold-start user has no mean or overlap evidence.',
    'interpret-damping': 'Confusing the link-following probability with the teleportation probability.',
    'run-power-iteration': 'Using the wrong matrix orientation or failing to split a node\'s outgoing mass.',
    'handle-dangling-node': 'Treating a dangling node as zero rank instead of declaring how its outgoing mass is redistributed.',
    'contrast-ranking': 'Ranking pages by indegree alone and ignoring source rank and outdegree.',
    'select-sketch': 'Choosing a sketch by name without matching it to the target statistic and query window.',
    'explain-dgim-error': 'Choosing the largest DGIM bucket instead of the oldest bucket cut by the query boundary.',
    'compute-fm-estimate': 'Reading the zero-prefix length R as a linear count instead of applying the exponential estimate.',
    'compute-second-moment': 'Confusing the number of distinct values with F2, the sum of squared frequencies.',
    'explain-ams-averaging': 'Treating one AMS sample as exact instead of averaging noisy estimators of the same moment.',
    'recall-balance-rule': 'Comparing initial budgets or ineligible bidders instead of remaining budgets of eligible advertisers.',
    'simulate-balance': 'Treating one tie outcome as enough to establish a guarantee for BALANCE.',
    'find-counterexample': 'Claiming non-optimality without checking OPT and the worst allowed tie path.',
    'reason-about-guarantees': 'Equating one successful simulation with a guarantee over all allowed tie resolutions.'
  }[skill] || `Leaving the ${skill} condition implicit when deciding whether the answer is valid.`),
  visualHook: extra.visualHook || ({
    'distinguish-linkages': 'Draw the closest and farthest cross-cluster pairs and label min, max, and average.',
    'calculate-distortion': 'Place 1, 3, and 5 on a number line, mark the mean, and sum squared distances.',
    'reason-about-local-optima': 'Separate assignment and mean-update arrows, then mark the local-minimum caveat.',
    'classify-bfr-state': 'Route the point through the DS, CS, and RS decision gates.',
    'compare-cluster-representations': 'Draw dispersed representatives, shrink them toward the mean, and keep several points.',
    'evaluate-clusters': 'Compare singleton and coarse partitions under purity and a second quality criterion.',
    'normalize-ratings': 'Subtract row means and circle only the co-rated centered entries.',
    'compute-dot-product': 'Write each coordinate product separately before adding the signed terms.',
    'separate-models': 'Draw two lanes: centered neighbors versus the latent q·p prediction.',
    'diagnose-cold-start': 'Show the empty user row and mark the missing mean and overlap prerequisites.',
    'interpret-damping': 'Split the update into a link-following lane and a teleportation lane.',
    'run-power-iteration': 'Draw outgoing arrows, split the source mass, and choose row or column orientation.',
    'handle-dangling-node': 'Circle the empty outgoing row and replace it with the declared redistribution vector.',
    'contrast-ranking': 'Annotate incoming arrows with source rank divided by source outdegree.',
    'select-sketch': 'Put DGIM, FM, and AMS under their target quantities before calculating.',
    'explain-dgim-error': 'Mark the window boundary and shade only the oldest partially covered bucket.',
    'compute-fm-estimate': 'Circle the zero prefix in the hash output before applying the power of two.',
    'compute-second-moment': 'Make a frequency table, square each frequency, and add the squares.',
    'explain-ams-averaging': 'Draw several sampled positions and combine their estimates with the stated average.',
    'recall-balance-rule': 'Keep a budget ledger with eligibility and remaining budget for each query.',
    'simulate-balance': 'Branch at the tie and compare each resulting leaf with OPT.',
    'find-counterexample': 'Draw both first-tie branches, record served queries, and compare with OPT.',
    'reason-about-guarantees': 'Use two columns: observed run versus every path required by a guarantee.'
  }[skill] || `Draw the ${skill} state before calculating.`),
  courseId: 'DSA5101',
  lessonId,
...
const assessmentMap = {
  ...
  summary: 'An evidence-linked revision map. Lectures define the core theory; official Assignments 1–2 provide the current exercise signal for all seven mapped topics. MMDS is attached as textbook depth and never silently upgraded to lecture scope.',
  disclaimer: 'A+ focus marks topics with current official lecture or assignment evidence and is a study priority, not a promise about final-exam questions. The Canvas project brief/rubric and detailed final-exam scope remain unverified.',
  studyOrder: [
