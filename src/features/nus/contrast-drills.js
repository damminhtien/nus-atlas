...
(function (root, factory) {
  ...
  function renderQuestion() {
    ...
    const feedback = answer ? `<div class="nus-contrast-feedback ${answer.correct ? "is-correct" : "is-wrong"}" role="status"><strong>${answer.correct ? "Correct distinction" : "Not quite"}</strong><p>${text(drill.explanation)}</p>${drill.misconception ? `<p class="nus-muted"><b>Common trap:</b> ${text(drill.misconception)}</p>` : ""}${drill.repair ? `<p class="nus-muted"><b>Repair rule:</b> ${text(drill.repair)}</p>` : ""}${!answer.correct ? `<p class="nus-muted">Correct answer: <b>${text(drill.choices[drill.answer])}</b></p>` : ""}</div>` : "";
