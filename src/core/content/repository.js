...
(function (root, factory) {
  ...
  function registerLesson(courseId, lessonId, payload) {
    const lesson = { ...(payload.lesson || payload), questions: payload.questions || [], ...(payload.studyKit || {}) };
  ...
  function listLessons(courseId) {
    const packageData = packageFor(courseId);
    if (packageData && packageData.content) return (packageData.content.modules || []).flatMap(module => (module.lessons || []).map(lesson => ({ ...lesson })));
    return getCatalog(courseId).modules.flatMap(module => module.lessons || []);
  }
  ...
  function getAssessment(courseId) { const packageData = packageFor(courseId); return packageData && Array.isArray(packageData.assessments) ? packageData.assessments.slice() : []; }
  ...
  function listLabs(courseId) { const packageData = packageFor(courseId); return packageData ? Object.entries(packageData.labs || {}).map(([id, lab]) => ({ id, ...lab })) : []; }
  ...
  function getSchedule(courseId) { return courseId ? schedule[courseId] || null : { courses: schedule }; }
  ...
  return Object.freeze({ listCourses, getCourse, getCourseOutline, getCatalog, listLessons, getLesson, loadCourse, loadLesson, getQuestions, getStudyKit, needsLoad, getAssessment, getAssessmentMap, getLab, listLabs, getVisual, listVisuals, getSchedule, getSourceTypes, getSourceCatalog, getTextbook, getQuestionBank, getSourceManifest, getSlideSets, getSlideSet, getSlide, stats });
});
