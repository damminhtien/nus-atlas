(function () {
  ...
  const shell = (lab, body) => `<section class="nus-lab nus-lab-${esc(lab.type)} reveal" data-nus-lab="${esc(lab.lessonId)}" data-reduced-motion="${lab.reducedMotion ? "true" : "false"}" aria-labelledby="nus-lab-title-${esc(lab.lessonId)}"><header class="nus-lab-head"><div><span class="pill violet">Visual learning lab</span><h3 id="nus-lab-title-${esc(lab.lessonId)}">${esc(lab.title)}</h3></div><span class="nus-lab-status" data-lab-status aria-live="polite">Not attempted</span></header><p class="nus-lab-goal"><b>Learning goal</b> ${esc(lab.learningGoal)}</p><div class="nus-lab-links">${routeLink("Course map", `#/nus/course/${lab.courseCode}`)}${routeLink("Practice this lesson", `#/nus/exam/${lab.courseCode}/${lab.lessonId}`, "primary")}</div>${body}<footer class="nus-lab-foot"><div><b>Source lens</b><span class="nus-lab-source-note">Lecture remains the exam-priority core. Textbook and reference steps are labeled for depth.</span></div><div class="nus-lab-sources">${sourceRefs(lab)}</div><details><summary>Why this interaction?</summary><p>${esc(lab.explanation || "Use the controls to make one explicit reasoning move, then explain the evidence.")}</p></details></footer></section>`;
